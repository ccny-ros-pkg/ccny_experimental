...
class EKF
{
  ...
  private:
    ...
    void getPrediction(float wx, float wy, float wz, float dt);
    void RotFromQuat(Eigen::Matrix<float, 3, 3>& R);
    void update(float ax, float ay, float az, float dt);
    void updateWithMag(float ax, float ay, float az, float mx, float my, float mz, float dt);
 void getMeasurement(
