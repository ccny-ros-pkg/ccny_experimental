...
EKF::EKF(const ros::NodeHandle& nh, const ros::NodeHandle& nh_private):
  nh_(nh), 
  ...
  q1_(0.0), q2_(0.0), q3_(0.0),  q4_(1.0)
{
  ...
  imu_EKF_publisher_ = nh_.advertise<sensor_msgs::Imu>("imu_EKF/data", queue_size);
...
void EKF::imuCallback(const ImuMsg::ConstPtr& imu_msg_raw)
{
...
double EKF::getInclination(double ax, double ay, double az, double mx, double my, double mz)
{
...
void EKF::prediction(double p, double q, double r, double dt)
{
